...
class MaritimeRouter:
    def __init__(self, min_lat, max_lat, min_lon, max_lon, step_m=10000, cache_dir="data"):
        self.cache_full = os.path.join(cache_dir, "graph_full.pkl")
        self.graph = None
        self.kdtree = None
    ...
    def route(self, start, end, speed_predictor, risk_zones=None, optimization='time', vessel_type='cargo', wind_speed=15, wave_height=3, season=1):
            ...
            if optimization == 'time':
                ...
                if risk_zones:
                    ...
                    for zone in risk_zones:
                        zone_center = (zone['center'][0], zone['center'][1])
                        d = haversine(center[0], center[1], zone_center[0], zone_center[1])
                        if d < zone['radius_km']:
                            risk_penalty += zone['avg_risk']
